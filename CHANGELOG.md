...
- **`detect` reports parameters initialized outside their declared range** -- A gen~ export can declare a range its own initial value violates, because the generated initializer writes the member directly while only the setter clamps (gigaverb's `revtime` inits to 11 with a declared maximum of 1). gen-dsp has always clamped such a value -- as gen~ itself would on the first parameter write -- but did so silently in two separate layers. `gen-dsp detect <export>` now lists each affected parameter with its initial value, declared range and clamped result, and `--json` carries the same under `out_of_range_params`, so the discrepancy reads as what it is: a bug in the source patch, fixable only in Max. The pre-clamp value is kept on `ParamInfo.raw_default` (`None` when the export is self-consistent) and deliberately left out of `to_dict()`, since it is a parse-time diagnostic rather than IR the backends consume -- the manifest JSON schema is unchanged.
...
- **CLAP: loading state no longer leaves the host showing stale parameter values** -- `clap_plugin_state::load()` wrote the restored values into the gen state without telling the host, so a host that caches parameter values (all of them do) kept displaying and automating the pre-load ones until something else forced a refresh. It now calls `clap_host_params::rescan(CLAP_PARAM_RESCAN_VALUES)`, which is safe to do inline because `load()` runs on the main thread. This is what clap-validator's `state-reproducibility-basic`/`-binary`/`-buffered` checks were failing on.

- **CLAP: the reported parameter default is now a constant** -- `params_get_info()` returned the parameter's *current* value as `default_value`, clamped into the declared range. A default that moves is a contract violation: CLAP hosts treat a changed default as requiring a full parameter rescan, so every state load looked like a plugin redefinition. It also disagreed with `params_get_value()` whenever a gen~ initial value sat outside its declared range, since only the reported default was clamped and never the live value. Both are fixed by clamping the live value once at creation and snapshotting the result, so the default is fixed for the lifetime of the instance and always agrees with what the host reads back.

- **CLAP: parameter values survive activation** -- `activate()` destroys and rebuilds the gen state for the new sample rate and block size, which resets every parameter to its gen~ initial value. Anything the host had set -- automation, or state loaded before activation, which is the normal order for a session reload -- was silently discarded. The current values are now carried across the rebuild, matching what the AudioUnit backend already did.

- **AudioUnit: parameter values are clamped to their declared range at creation** -- Same underlying gen~ quirk as the CLAP default fix above: the plugin reported a clamped default through `kAudioUnitProperty_ParameterInfo` while actually running at the raw initial value, and the two converged only when something first wrote the parameter -- an audible jump on a patch like gigaverb. The live values are now pulled into range in the factory, before any host query, and `AUGenInitialize()` carries them across re-initialization as before. The VST3 and LV2 backends were already immune: VST3 writes every parameter from the (clamped) controller default in `setActive()`, and LV2 writes its control ports into the gen state on every `run()`.

- **Graph: SVF filters no longer produce non-finite output below ~4 kHz** -- The TPT prewarp `tan(pi * f / sr)` diverges as the cutoff approaches Nyquist, so a graph whose cutoff exceeds `sr/2` -- easy to reach at a low sample rate, and hosts probe down to 1 kHz -- emitted `inf` from the first block. The `subtractive_synth` example (2 kHz cutoff) did exactly this at 1234.57 Hz. The prewarp frequency is now clamped to `0.49 * sr` in both the C++ emitter and the Python simulator, so the two stay in agreement.

- **The CLAP validation helper could never pass** -- `validate_clap()` asserted `"0 failed" in result.stdout`, but clap-validator colours its summary line unconditionally (honouring neither `NO_COLOR` nor a non-tty stdout), so the literal text was never present and every CLAP build-integration test failed wherever cargo was available. The helper now strips ANSI escapes before parsing the counts, and fails loudly if the summary line cannot be found at all rather than passing by accident. It also tolerates -- and only -- failures the validator self-diagnoses as its own bug, which the pinned revision hits on any plugin exposing zero parameters (it divides by the parameter count in `param-conversions`).
...
- **New graph node types and operators (gen~ coverage, Tier 1)** -- Four additions widen the graph frontend's gen~ coverage, each wired through every layer (model, C++ emitter, simulator, constant-fold/CSE, gen~ transpiler, differential evaluator, `.gdsp` DSL, serializer, visualizer) with a dedicated test module and differential-corpus cases. (1) Bitwise operators `bitand`/`bitor`/`bitxor`/`bitshift` (on `BinOp`) and `bitnot` (on `UnaryOp`), operating on the 32-bit-integer value of their float operands; the scalar semantics live once in `gen_dsp.graph.bitops` so the three Python evaluators (simulate, fold, transpile-eval) cannot drift, and the emitted GenExpr uses gen~'s named bitwise operators. (2) An `interp` node with `linear` and `cosine` modes -- the two-point interpolation `Mix` (linear-only) could not express. (3) A `nearest` interpolation mode on `BufRead`/`DelayRead` (round-half-up index versus `none`'s truncation). (4) A `train` impulse-generator node emitting a one-sample `1.0` each cycle at a given frequency, a trigger source distinct from the bipolar `PulseOsc`. gen~'s `rate` and four-point cubic/spline `interp` are intentionally deferred (ambiguous or Max-unverifiable semantics; `RateDiv` and the existing `buf_read`/`delay_read` cubic paths already cover the practical cases). The gen~ `bitshift` direction is a defined-but-Max-unverified convention (non-negative shifts left, negative right, amount masked to `[0, 31]`), consistent across all emitters so the differential harness stays meaningful. Five demo patches in `examples/dsl/` exercise the new operators -- `bitcrush` (bit-depth reduction), `bitglitch` (XOR/OR oscillator cross-modulation), `sh_sequencer` (`train`-clocked sample-and-hold), `wavemorph` (cosine `interp`), and `lofi_wavetable` (`nearest` buffer reads) -- each built end-to-end by the existing example-build test suite. See `docs/graph/coverage.md` for the full coverage analysis and the rejected-Tier-2 (multichannel / host-bound buffer) spike finding.
...
- **Project config file (`gen-dsp.toml`)** -- The default command now reads defaults from a `gen-dsp.toml` (auto-loaded from the current directory, or via `--config PATH`); a bare `gen-dsp` runs from it. Keys mirror the CLI options (hyphens or underscores), `platform` may be a list or `all`, and any CLI flag overrides the file. This makes repeated/CI builds reproducible without long flag strings. TOML is read with the standard-library `tomllib` on Python 3.11+, falling back to `tomli` on 3.10 (with a clear error if absent), keeping the core dependency-free on modern Pythons. Unknown keys and bad value types are reported with clear errors. See `examples/gen-dsp.toml`. (REVIEW.md section 6.3)

- **Multi-target generation** -- The default command's `-p/--platform` now accepts a comma-separated list (`-p clap,vst3,au`) or `all`, generating (and building, unless `--no-build`) each target from a single parse of the source. With multiple targets, `-o` is treated as a parent directory (each target lands in `<o>/<name>_<platform>`); single-target behavior is unchanged. A per-target summary is printed and the command exits non-zero if any target fails, while continuing past failures (e.g. a graph target a platform does not support). (REVIEW.md section 6.2)

- **`gen-dsp doctor` command** -- New diagnostic that reports, per platform, whether the host has the tools needed to build (CMake/Make, C/C++ compilers, ARM/AArch64 cross-toolchains, Emscripten, Xcode, git), with install hints for anything missing. Auto-fetched SDKs (CLAP/VST3/LV2/SC headers, Rack SDK, libDaisy, Circle, miniaudio) are reported as informational notes rather than hard prerequisites. Supports `-p <platform>` to check one platform and `--json` for machine-readable output; exits non-zero when a requested platform is not ready, so it works as a CI gate. Requirements live in `gen_dsp/core/doctor.py`. (REVIEW.md section 6.1)
...
- **Single version source of truth** -- Consolidated the three divergent version strings -- the package `__version__`, `gen_dsp.graph.__version__` (was `0.1.6`), and `Platform.GENEXT_VERSION` (was `0.8.0`, also the `Manifest` default) -- into one dependency-free `gen_dsp/version.py` (`__version__`), imported everywhere (a leaf module, so it is safe to import during package initialization). Generated projects are now stamped with the package version (`0.2.0`) instead of `0.8.0`. The generated build-file/C define and its template plumbing were renamed `GEN_EXT_VERSION`/`genext_version` -> `GENDSP_VERSION`/`gendsp_version`.

- **Normalized internal "dsp-graph" naming to "graph"** -- The in-tree graph backend referred to itself as "dsp-graph" -- the name of the separate companion web app ([shakfu/dsp-graph](https://github.com/shakfu/dsp-graph)) -- which was confusing. Internal identifiers, the manifest `source` value (`"dsp-graph"` -> `"graph"`), CLI source labels, the `_require_dsp_graph()` -> `_require_graph()` guard, and generated-file comments now use "graph"/`gen_dsp.graph`. The `pip install gen-dsp[graph]` install hint and the external-repo links are unchanged.

- **Clearer GDSP DSL diagnostics for builtin calls** -- Builtin calls (`clamp`, `select`, `scale`, oscillators, etc.) now report argument errors as `GDSPCompileError` with `line:col` instead of silently dropping extra arguments or surfacing a raw pydantic error. Too many positional args, unknown keyword args, and missing required args each give a precise message (e.g. `'clamp' takes at most 3 argument(s) (a, lo, hi), got 4`). (REVIEW.md section 6.7)

- **`gen-dsp detect` now introspects graph files too** -- `detect` auto-detects its argument: a directory is analyzed as a gen~ export (as before), while a `.gdsp`/`.json` file is analyzed as a graph, reporting name, I/O, parameters (with ranges), a node-type breakdown, buffers, delay lines, and validity (text or `--json`). The graph-loading logic is now a shared `_load_graph_file()` helper used by both `detect` and the default command. (REVIEW.md section 6.6)
...
- **De-duplicated the CLI default-command handlers** -- The identical build-or-print-next-steps tail of the gen~-export and graph default commands is now a single `_build_or_next_steps()` helper in `cli.py`. (REVIEW.md section 5.5)

- **Split the three large modules into packages** -- `graph/compile.py` (1776 LOC) -> `graph/compile/` (`common`, `state`, `nodes`, `__init__`); `graph/dsl.py` (1881 LOC) -> `graph/dsl/` (`lexer`, `parser`, `lower`, `__init__`); `platforms/circle.py` (1827 LOC) -> `platforms/circle/` (`boards`, `sdk`, `chain`, `dag`, `__init__`). Each package's `__init__` preserves the existing public import paths (e.g. `from gen_dsp.graph.compile import compile_graph`, `from gen_dsp.platforms.circle import CirclePlatform`) so no call sites changed. Pure reorganization with no behavior change: generated C++ and project files are byte-for-byte identical across all example graphs and platforms. (REVIEW.md section 5.4)

- **Shared platform machinery lifted into the `Platform` base class** -- Replaced per-module reimplementations with shared helpers: `render_template()` (supersedes the four differently-named template renderers `_generate_from_template`/`_render_template` and backs every `_generate_cmakelists`/`_generate_makefile`, so CMake/Make platforms now only assemble the variable dict); `find_output_by_pattern()` (a consistent glob-with-missing-output policy adopted by 14 of 15 platforms); and `capitalize_first()` + `sanitize_c_identifier()` (deduplicating the copies in ChucK/SuperCollider/LV2). Generated output is byte-for-byte identical across the export and graph paths for all platforms. (REVIEW.md section 5.3)
...
- **VST3: broaden cross-compile guard for env-script SDKs** -- The `moduleinfotool` guard in the VST3 template now also disables when the `CROSS_COMPILE` environment variable is set, not only when `CMAKE_CROSSCOMPILING` is true. Env-script cross toolchains (e.g. Elk's `source environment-setup; cmake -B build`) export `CROSS_COMPILE` but can leave `CMAKE_CROSSCOMPILING` unset, so the guard previously did not fire for them and the ARM-built `moduleinfotool` failed to run on the x86 host. Also added regression tests for the codebox buffer over-counting case (`Data` buffers passed as function arguments). ([#5](https://github.com/shakfu/gen-dsp/issues/5), [#6](https://github.com/shakfu/gen-dsp/issues/6))

- **AUv3: buffer struct casing** -- `auv3_buffer.h` defined `Auv3Buffer` while `_ext_auv3.cpp` instantiated `AUv3Buffer`, so any AUv3 plugin *with buffers* failed to compile (thanks @caitp, [#8](https://github.com/shakfu/gen-dsp/pull/8)). Added regression tests in `tests/test_auv3.py`: a non-gated generation check that the buffer struct name in the header matches what the adapter instantiates (runs on every platform), plus an AUv3-with-buffers build-integration test (macOS + Xcode). The existing gigaverb build test could not catch this -- with no buffers the adapter's buffer branch never compiles.
