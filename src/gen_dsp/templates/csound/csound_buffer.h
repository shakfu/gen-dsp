// csound_buffer.h - Buffer class for gen~ code (genlib side)
// Uses DataInterface for gen~ compatibility

#ifndef CSOUND_BUFFER_H
#define CSOUND_BUFFER_H

#include "genlib.h"

// CsoundBuffer - buffer wrapper for gen~ DataInterface
// Buffers are allocated locally; data is zero-filled by default.
struct CsoundBuffer : public DataInterface<t_sample> {

    CsoundBuffer() : DataInterface<t_sample>() {
        mData = nullptr;
        mOwnedData = nullptr;
        dim = 0;
        channels = 1;
