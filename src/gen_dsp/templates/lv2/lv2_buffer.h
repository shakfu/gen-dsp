// lv2_buffer.h - Buffer class for gen~ code (genlib side)
// Uses DataInterface for gen~ compatibility, no LV2 headers
...
// Lv2Buffer - buffer wrapper for gen~ DataInterface
// Buffers are allocated locally; data is zero-filled by default.
struct Lv2Buffer : public DataInterface<t_sample> {

    Lv2Buffer() : DataInterface<t_sample>() {
        mData = nullptr;
        mOwnedData = nullptr;
        dim = 0;
        channels = 1;
