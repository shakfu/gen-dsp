// standalone_buffer.h - Buffer class for gen~ code (genlib side)
// Uses DataInterface for gen~ compatibility

#ifndef STANDALONE_BUFFER_H
#define STANDALONE_BUFFER_H

#include "genlib.h"

// StandaloneBuffer - buffer wrapper for gen~ DataInterface
// Buffers are allocated locally; data is zero-filled by default.
struct StandaloneBuffer : public DataInterface<t_sample> {

    StandaloneBuffer() : DataInterface<t_sample>() {
        mData = nullptr;
        mOwnedData = nullptr;
        dim = 0;
        channels = 1;
