// vcvrack_buffer.h - Buffer class for gen~ code (genlib side)
// Uses DataInterface for gen~ compatibility, no VCV Rack headers

#ifndef VCVRACK_BUFFER_H
#define VCVRACK_BUFFER_H

#include "genlib.h"

// VcvrackBuffer - buffer wrapper for gen~ DataInterface
// Buffers are allocated locally; data is zero-filled by default.
struct VcvrackBuffer : public DataInterface<t_sample> {

    VcvrackBuffer() : DataInterface<t_sample>() {
        mData = nullptr;
        mOwnedData = nullptr;
        dim = 0;
        channels = 1;
