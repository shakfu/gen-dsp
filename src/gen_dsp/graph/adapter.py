"""gen-dsp adapter: bridge graph output to gen-dsp platform backends.

Generates adapter C++ code that implements gen-dsp's ``_ext.h`` wrapper
interface by delegating to the graph compiled API.  Also generates
...
def generate_adapter_cpp(graph: Graph, platform: str) -> str:
    ...
    This replaces the genlib-side wrapper with graph calls while
    ...
    w(f"// _ext_{platform}.cpp -- graph adapter for gen-dsp {platform} backend")
    w("// Generated by gen-dsp (graph frontend)")
    w("")
    w(f'#include "gen_ext_common_{platform}.h"')
    w("")
    w("// Include graph compiled code")
...
def generate_graph_build_file(
    ...
    num_outputs: int,
    num_params: int = 0,
    gendsp_version: str = __version__,
    shared_cache: bool = False,
    midi_defines: str = "",
) -> Path:
    """Generate a simplified build file for a graph project.

    Unlike gen~ projects, graph projects have no genlib dependency,
    ...
    Args:
        ...
        gendsp_version: gen-dsp version string.
...
# Generated by gen-dsp (graph source)

set(PROJECT_NAME {lib_name})
project(${{PROJECT_NAME}} VERSION {gendsp_version})
...
def _cmake_clap(**kwargs: object) -> Path:
    ...
    gendsp_version = str(kwargs["gendsp_version"])
    shared_cache = bool(kwargs["shared_cache"])
    midi_defines = str(kwargs["midi_defines"])

    content = _cmake_common_header(lib_name, gendsp_version, "clap", shared_cache)
    content += """
...
if(APPLE)
    set_target_properties(${{PROJECT_NAME}} PROPERTIES
        ...
        MACOSX_BUNDLE_BUNDLE_VERSION "{gendsp_version}"
    )
...
def _cmake_vst3(**kwargs: object) -> Path:
    ...
    content = _cmake_common_header(lib_name, gendsp_version, "vst3", shared_cache)
    content += f"""
...
def _cmake_lv2(**kwargs: object) -> Path:
    ...
    content = _cmake_common_header(lib_name, gendsp_version, "lv2", shared_cache)
    content += f"""
include(FetchContent)
FetchContent_Declare(
    lv2
...
def _cmake_sc(**kwargs: object) -> Path:
    ...
    content = _cmake_common_header(lib_name, gendsp_version, "sc", shared_cache)
    content += f"""
include(FetchContent)
FetchContent_Declare(
    supercollider
...
cflags = -I. \\
    ...
    -DGENDSP_VERSION="{gendsp_version}" \\
...
FLAGS+=-DGENDSP_VERSION="{gendsp_version}"
...
CFLAGS += -DGENDSP_VERSION=\\"{gendsp_version}\\"
...
CPPFLAGS += -DGENDSP_VERSION=\\"{gendsp_version}\\"
...
def _generate_buffer_header(output_dir: Path) -> None:
    """Generate a gen_buffer.h with zero buffers (graph manages its own)."""
    content = (
        "// gen_buffer.h -- auto-generated by graph\n"
        "#ifndef GEN_BUFFER_H\n"
        "#define GEN_BUFFER_H\n"
        "\n"
