# Adding New Backends to gen_ext

This guide explains how to add support for a new audio platform (e.g., SuperCollider, VCV Rack, LV2) to gen_ext.

## Overview

gen_ext uses a platform registry system that makes adding new backends straightforward. Each platform is a Python class that implements the `Platform` interface, plus a set of C++ template files for the actual wrapper code.
...
All platforms extend the `Platform` abstract base class in `src/gen_ext/platforms/base.py`:
...
Create a new file `src/gen_ext/platforms/yourplatform.py`:
...
class YourPlatform(Platform):
    ...
    name = "yourplatform"  # Used in CLI: gen-ext init -p yourplatform
...
Create directory `src/gen_ext/templates/yourplatform/` with:
...
Edit `src/gen_ext/templates/__init__.py` to add a function for your templates:
...
Edit `src/gen_ext/platforms/__init__.py`:

```python
from gen_ext.platforms.yourplatform import YourPlatform
...
- `gen-ext init -p yourplatform` will work
- `gen-ext build -p yourplatform` will work
